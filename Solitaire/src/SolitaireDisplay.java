...
/**
 * SolitaireDisplay draws the game board every time paint is called. </br>
 * Display also keeps tract of what is selected and calls Solitaire's methods accordingly when a click is made. </br>
 * Provides methods for game to modify which cards of the waste pile is displayed through {@code addWaste(), clearWaste(), removeWaste()}. </br>
 * Also implements Runnable and can be started as a separate thread to constantly update elapsed time and display it. </br>
 * 
 * @author Dennis Moon
 * @version Mar 12, 2013
 * 
 ...
 *     Mar 6, 2013 - double click is shortcut for one card to foundation, right click is shortcut for all cards to foundation hovering over a card now highlights it. 
 *     Mar 11, 2013 - clearWastes() modified for proper undo() of game.dealThreeCards() </br>
 *     Mar 12, 2013 - modified frame & added label for time, implemented Runnable & run() to update time
 */
public class SolitaireDisplay extends JComponent implements MouseListener, MouseMotionListener, Runnable
{
	...
	/**
	 * <b>Constructor: </b>SolitaireDisplay</br> 
	 * <b>Usage: <b>{@code SolitaireDisplay display = new SolitaireDisplay(game)}</br>
	 * -------------------------------</br>
	 * Creates a new frame and inserts itself as a canvas into the frame. </br>
	 * Frame listens to a keyListener (the game) and this listens to mouse click & motion. </br>
	 * Frame's layout is set as absolute: this & label are positioned & sized accordingly. </br>
	 * After the frame is shown, startTime is calculated and a new thread is started for displaying time. </br>
	 * 
	 * @param game - Solitaire game
	 */
	...
	{   
		...
		if (hoverRow == 0 && hoverCol == 0) //draws a yello border around stock when cursor is hovered
            drawBorder(g, Color.YELLOW, 0, SPACING, SPACING);

		//waste
		for (int i=0; i<wastes.size(); i++) //displays waste cards according to size of wastes
		    drawCard(g, wastes.get(i), SPACING * 2 + CARD_WIDTH + (i*FACE_UP_OFFSET), SPACING);
		if (selectedRow == 0 && selectedCol == 1 && wastes.size()!=0) //draws orange border if waste is selected
			drawBorder(g, Color.ORANGE, 0, SPACING * 2 + CARD_WIDTH + ((wastes.size()-1)*FACE_UP_OFFSET), SPACING);
		else if (hoverRow == 0 && hoverCol == 1 && wastes.size()!=0) //draws yellow border if waste is hovered
		    drawBorder(g, Color.YELLOW, 0, SPACING * 2 + CARD_WIDTH + ((wastes.size()-1)*FACE_UP_OFFSET), SPACING);
		    
		//foundation
		for (int i = 0; i < 4; i++) //draws each foundation stack of cards
			drawCard(g, game.getFoundationCard(i), SPACING * (4 + i) + CARD_WIDTH * (3 + i), SPACING);
		if (selectedRow == 0 && selectedCol >= 3 && game.getFoundationCard(selectedCol-3)!=null) //draws orange border if foundation is selected
		    drawBorder(g, Color.ORANGE, 0, SPACING * (4 + selectedCol-3) + CARD_WIDTH * (3 + selectedCol-3), SPACING);
		else if (hoverRow == 0 && hoverCol >= 3 && game.getFoundationCard(hoverCol-3)!=null) //draws yellow border if foundation is hovered
		...
		for (int i = 0; i < 7; i++)
		{
			...
			for (int j = 0; j < pile.size(); j++)
			{
				...
				if (selectedRow == 1 && selectedCol == i && j == pile.size() - 1) //draws orange border if foundation is selected
				...
				else if (hoverRow == 1 && hoverCol == i && j == pile.size() - 1) //draws yellow border if foundation is selected
	...
	public void run()
	{
	    while(true)
	    {
	        label.setText("Time: " + timeElapsed);
	        timeElapsed++;
	...
	/**
	 * <b>Method: </b>setStartTime</br> 
	 * <b>Usage: </b>{@code }</br>
	 * -------------------------------</br>
	 * Sets a new startTime for when a new game is called. </br>
	 * 
	 * @param t new startTime in ms
	 */
