...
public class Tetrad
{
     ...
     * @return newLoc - locations of the blocks to be inserted into
     */
    private Location[] setLocsInSubGrid()
    {        
        Location[] newLoc = new Location[4];
        if (type == LONG) //Long tetrad
        {
    ...
    /**
     * Method: addToMain()
     * Usage: active.addToMain()
     * --------------------------
     * puts this tetrad into the main game board
     * @return true if it does not overlap already existing tetrads
     */
    public boolean addToMain()
    {
        try
        {
            ...
            Location[] locsInMain = setLocsInMain();
            for (Location l : locsInMain)
                if (grid.get(l)!=null)
                    return false;
            
            addToLocations(grid, locsInMain);
            updateShadow();
            return true;
        }
    ...
    /**
     * Method: addToSubGrid(gr)
     * Usage: hold.addToSubGrid(holdGrid)
     * ------------------------------------
     * puts this tetrad into the hold/next grid 
     * @param gr - hold/next grid to be put into
     */
    public void addToSubGrid(MyBoundedGrid<Block> gr)
    {
        try
        {
            lock.acquire();
            ...
            removeShadow();
            addToLocations(gr, setLocsInSubGrid());
        }
        catch (InterruptedException e){}
        finally
        {
     ...
     * -----------------------------------------------
     * translates the tetrad in the given direction
     * it first removes the shadow, translates, then updates the shadow afters
     ...
     * removes the shadow first, rotates, then updates shadow
    ...
    public boolean rotate()
    {
        ...
        try
        {
            ...
            if (!areValid(grid, newLocs))
                 newLocs = fixRotation(grid, newLocs);
     ...
     * then the corrected locations are returned
     * @param gr - grid this tetrad is to be fixed
     * @param locs - locations to be fixed
     * @return locs - the fixed or unfixed locs 
     */
    private Location[] fixRotation(MyBoundedGrid<Block> gr, Location[] locs)
    {
    ...
    /**
     * Method: getType()
     * Usage: tetrad.getType()
     * --------------------------
     * @return - type of this tetrad
     */
    public int getType()
    {
        return type;
