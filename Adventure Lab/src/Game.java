 ...
 *          every monster in the hero's room attacks hero with 1/3 chance
 *          if hero dies, user is prompted if they want to start again: setup is called again
 *          
...
public class Game
{
    ...
    {   
        ...
        smallPotions[2].changeOwner(hero);
        smallPotions[3].changeOwner(slime);
    ...
    public static String makeQuantityList (ArrayList<Thing> things)
    {
        ...
        {
            count = 1;
            //traverses arraylist backwards to find and remove item with same name, then increments count
            for (int n=copy.size()-1; n>i; n--)
            {
                if (copy.get(i).equals(copy.get(n)))
                {
                    count++;
                    copy.remove(n);
                }
            ...
            //if count is bigger, the quantity is added
            if (count>1)
                copy.set(i, copy.get(i)+"(" + count + ")");
        }
